...
if  __name__ == "__main__":
    ...
    for box in boxes:
      ...
      normal_boxes = [round(x1), round(y1), round(x2), round(y2),box[4],box[5],box[6]]
      new_boxes.append(normal_boxes)
    

    row = imgfile + "," + "".join(str(e) for e in new_boxes) + "\n"
    #print(row)
    save_path = f"Yolov4_Deep_learn{weightfile}.txt"
    with open(save_path.replace("/",""),"a")as f:
      f.write(row)

    #output_path = "prediction.csv"
    #predictions = pd.DataFrame([class_names,img,weightfile], columns=["class_names","image","weight"]) 
    #predictions.to_csv(output_path, mode='a', header=not os.path.exists(output_path), index=False)
